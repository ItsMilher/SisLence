...
if ($_POST['action'] == 'searchCliente') {
  if (!empty($_POST['cliente'])) {
    $nombreCL = $_POST['cliente'];

    $query = mysqli_query($conexion, "SELECT * FROM cliente WHERE nombre LIKE '$nombreCL'");
    mysqli_close($conexion);
    $result = mysqli_num_rows($query);
    $data = '';
    if ($result > 0) {
...
if ($_POST['action'] == 'procesarVenta') {
  if (empty($_POST['codcliente']) or empty($_POST['pago'])) {
    $codcliente = 1;
    $pago = 0;
  }else{
  ...
  if ($result > 0) {
    $query_procesar = mysqli_query($conexion, "CALL procesar_venta($usuario,$codcliente,'$token',$pago)");
